...
public class CuentasCompartidasVistaControlador extends MenuLateralControladorPlantilla {
    ...
    private void actualizarTituloCuenta() {
        if (lblTituloCuenta != null) {
            TipoCuenta cuenta = Configuracion.getInstancia().getGestionGastos().getCuentaActiva();
            if (cuenta != null) {
                lblTituloCuenta.setText("Cuenta Compartida: " + cuenta.getNombre());
            }
        }
    }
    ...
    public void botonCategorias(ActionEvent event) {
        System.out.println(">>> Acción: Categorías (Compartida)");
